...
class _F:
    ...
    def __call__(self, preds, labels, task, by_class=False):
        assert(len(preds.shape) == 2 and len(labels.shape) == 1) 
        num_classes = preds.shape[-1]
        if by_class:
            return self.metric(preds, labels, task, num_classes=num_classes, average=None)
        assert preds.device == labels.device, f"Expected preds and labels on same device. preds:{preds.device} labels:{labels.device}"
        return self.metric(preds, labels, task, num_classes=num_classes, average=self.avg)
...
def _gen_primary_model_metrics(stats, preds, labels, task, by_class):
    ...
    with torch.no_grad():
        for metric_name, metric in metrics:
            stats[metric_name] = metric(preds, labels, task).item()
            if not by_class:
                continue
            by_class_metric = metric(preds, labels, task, by_class=True)
...
def _gen_ensemble_metrics(stats, preds, labels, task):
    ...
    with torch.no_grad():
        for metric_name, metric in metrics:
            ...
            for i in range(preds.shape[1]):
                results.append(metric(preds[:,i,:], labels, task))
...
def _gen_individual_model_metrics(stats, preds, labels, task):
    _assert_data_fit_for_metrics(preds, labels, expected_pred_dims=3)
    ...
    with torch.no_grad():
        for idx in range(preds.shape[1]):
            ...
            stats[model_id] = dict()
            for metric_name, metric in metrics:
                stats[model_id][metric_name] = metric(preds[:,idx,:], labels, task).item()
...
def get_ensemble_metrics(preds, labels, task, by_class=True):
    _assert_data_fit_for_metrics(preds, labels, expected_pred_dims=3)
    ...
    _gen_ensemble_metrics(stats["Ensemble"], preds, labels, task)
    _gen_primary_model_metrics(stats["Soft Vote"], soft_vote_preds, labels, task, by_class=by_class)
    _gen_primary_model_metrics(stats["Hard Vote"], hard_vote_preds, labels, task, by_class=by_class)
    _gen_individual_model_metrics(stats["Individual"], preds, labels, task)
...
def eval_and_log_metrics(writer, preds, labels, args, task):
    ...
    stats = get_ensemble_metrics(preds, labels, task)
    for key, value in stats.items():
...
def eval_and_log_validation_metrics(writer, preds, teacher_preds, args, task, tag="Validation"):
    """Compute and log train metrics with writer for given preds, labels, and args"""
    labels = teacher_preds.argmax(dim=1)
    stats = get_ensemble_metrics(preds, labels, task, by_class=False)
