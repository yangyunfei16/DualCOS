...
# An improved version of perm ensures that the same sample is not repeatedly sampled.
def perm_b(pop_size, num_samples, used_list, selected_idx, device):
    """Use torch.randperm to generate num_samples indices on a tensor of size pop_size."""
    # Source: https://discuss.pytorch.org/t/torch-equivalent-of-numpy-random-choice/16146/19
    pop_size_list = list(range(pop_size))
    set_A = set(used_list)
    new_list = [item for item in selected_idx if item not in set_A]
    # random.shuffle(new_list)
    # print("the length of new_list: ", len(new_list))
    pop_size_b = torch.LongTensor(new_list).to(device)
    # return pop_size_b[:num_samples]
    return pop_size_b[:num_samples]
